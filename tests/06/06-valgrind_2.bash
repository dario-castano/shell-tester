...
if [[ $valgrind_check -eq 1 ]]; then
	...
	valgrind --leak-check=full ./$HSHELL < ls > test_ing.txt 2>&1


	# cheking for patters that indicate err
	grep -q "All heap blocks were freed -- no leaks are possible" "test_ing.txt"
	grep "All heap blocks were freed -- no leaks are possible" "test_ing.txt" > output.txt
